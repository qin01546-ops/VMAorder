...
coefficient matrix at lag $h$. It is mainly designed for exploratory and
methodological experiments with high-dimensional VMA-type processes.
...
The function `covVMA()` estimates the lagged cross-covariance matrix at
a given lag.
...
The function `statVMA()` computes the statistic values over a set of
candidate lags.
...
The function `orderVMA()` estimates the VMA order using a
stable-interval rule. The selected order is taken as one lag before the
statistic enters a stable interval.
...
<img src="man/figures/README-order-plot-example-1.png" alt="" width="100%" />
...
The function `stationVMA()` applies the Augmented Dickey-Fuller test to
each variable in the multivariate time series.
...
The function `plotVMA()` can be used to compare mean statistic curves
across different VMA orders and simulation settings.
